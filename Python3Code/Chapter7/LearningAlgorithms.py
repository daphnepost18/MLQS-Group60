...
class ClassificationAlgorithms:
    ...
    def feedforward_neural_network(self, train_X, train_y, test_X, hidden_layer_sizes=(100,), max_iter=500,
        ...
        if gridsearch:
            ...
            nn = GridSearchCV(MLPClassifier(), tuned_parameters, cv=3, scoring='accuracy')
        else:
            # Create the model
    ...
    def support_vector_machine_without_kernel(self, train_X, train_y, test_X, C=1, tol=1e-3, max_iter=1000,
        ...
        if gridsearch:
            tuned_parameters = [{'max_iter': [1000, 2000], 'tol': [1e-3, 1e-4],
                                 'C': [1, 10, 100]}]
            svm = GridSearchCV(LinearSVC(), tuned_parameters, cv=3, scoring='accuracy')
        else:
    ...
    def k_nearest_neighbor(self, train_X, train_y, test_X, n_neighbors=5, gridsearch=True, print_model_details=False):
        # Create the model
        if gridsearch:
            tuned_parameters = [{'n_neighbors': [1, 2, 5]}]
            knn = GridSearchCV(KNeighborsClassifier(), tuned_parameters, cv=3, scoring='accuracy')
        else:
    ...
    def decision_tree(self, train_X, train_y, test_X, min_samples_leaf=50, criterion='gini', print_model_details=False,
        ...
        if gridsearch:
            ...
            dtree = GridSearchCV(DecisionTreeClassifier(), tuned_parameters, cv=3, scoring='accuracy')
        else:
    ...
    def random_forest(self, train_X, train_y, test_X, n_estimators=10, min_samples_leaf=5, criterion='gini',
        ...
        if gridsearch:
            tuned_parameters = [{'min_samples_leaf': [2, 10, 50, 100, 200],
                                 ...
                                 'criterion': ['gini', 'entropy']}]
            rf = GridSearchCV(RandomForestClassifier(), tuned_parameters, cv=3, scoring='accuracy')
        else:
...
class RegressionAlgorithms:
    ...
    def feedforward_neural_network(self, train_X, train_y, test_X, hidden_layer_sizes=(100,), max_iter=500,
        ...
        if gridsearch:
            ...
            nn = GridSearchCV(MLPRegressor(), tuned_parameters, cv=3, scoring='neg_mean_squared_error')
        else:
            # Create the model
    ...
    def support_vector_regression_with_kernel(self, train_X, train_y, test_X, kernel='rbf', C=1, gamma=1e-3,
        ...
        if gridsearch:
            tuned_parameters = [{'kernel': ['rbf', 'poly'], 'gamma': [1e-3, 1e-4],
                                 'C': [1, 10, 100]}]
            svr = GridSearchCV(SVR(), tuned_parameters, cv=3, scoring='neg_mean_squared_error')
        else:
            # Create the model
    ...
    def support_vector_regression_without_kernel(self, train_X, train_y, test_X, C=1, tol=1e-3, max_iter=1000,
        ...
        if gridsearch:
            ...
            tuned_parameters = [{'max_iter': [1000, 2000], 'tol': [1e-3, 1e-4],
                                 'C': [1, 10, 100]}]
            svr = GridSearchCV(LinearSVR(), tuned_parameters, cv=3, scoring='neg_mean_squared_error')
        else:
            # Create the model
    ...
    def k_nearest_neighbor(self, train_X, train_y, test_X, n_neighbors=5, gridsearch=True, print_model_details=False):
        ...
        if gridsearch:
            tuned_parameters = [{'n_neighbors': [1, 2, 5]}]
            knn = GridSearchCV(KNeighborsRegressor(), tuned_parameters, cv=3, scoring='neg_mean_squared_error')
        else:
            # Create the model
    ...
    def decision_tree(self, train_X, train_y, test_X, min_samples_leaf=50, criterion='mse', print_model_details=False,
        ...
        if gridsearch:
            tuned_parameters = [{'min_samples_leaf': [2, 10, 50, 100, 200],
                                 'criterion': ['mse']}]
            dtree = GridSearchCV(DecisionTreeRegressor(), tuned_parameters, cv=3, scoring='neg_mean_squared_error')
        else:
            # Create the model
    ...
    def random_forest(self, train_X, train_y, test_X, n_estimators=10, min_samples_leaf=5, criterion='mse',
        ...
        if gridsearch:
            tuned_parameters = [{'min_samples_leaf': [2, 10, 50, 100, 200],
                                 'n_estimators': [10, 50, 100],
                                 'criterion': ['mse']}]
            rf = GridSearchCV(RandomForestRegressor(), tuned_parameters, cv=3, scoring='neg_mean_squared_error')
        else:
            # Create the model
